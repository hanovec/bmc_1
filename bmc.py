...
@st.cache_resource
def load_model():
    """Načte a cachuje Gemini model, aby se nenačítal při každém obnovení stránky."""
    ...
    try:
        ...
        if not model_name_to_use:
            st.error("Nebyl nalezen žádný z prioritních modelů.")
            return None
        loaded_model = genai.GenerativeModel(model_name=model_name_to_use, generation_config=GENERATION_CONFIG)
        return loaded_model
...
def ask_gemini_sdk_st(prompt_text: str, temperature: float = None) -> str:
    ...
    if temperature:
        spinner_text += f" (teplota: {temperature})"
...
# --- Logika Aplikace a Session State ---
...
if not model:
    st.error("Model se nepodařilo načíst. Zkontrolujte API klíč a konfiguraci.")
    st.stop()
...
if st.session_state.stage == 'WELCOME':
    ...
    if st.button("Potvrdit a zahájit analýzu"):
        if context:
            ...
            st.session_state.history.append({'role': 'user', 'content': context})
    ...
    if "AI_ERROR" not in response_text:
        try:
            ...
            st.error(f"Nepodařilo se zpracovat plán od AI: {e}")
    else:
    ...
    if q_index < len(st.session_state.question_plan):
        ...
        ai_box(q_config.get('question', 'Chybí text otázky.'), title=f"Oblast {q_index+1}: {q_config.get('key', '').replace('_', ' ').title()}")
        
        with st.expander("Pro komplexní odpověď zvažte tyto body"):
    ...
    st.session_state.innovation_list = innovation_list_str
    ...
    ai_box(f"Nyní detailně rozpracuji {len(st.session_state.innovation_titles)} nápadů.", "Detailní rozpracování")
    bmc_summary_str = "\n".join([f"- {k}: {v}" for k, v in st.session_state.bmc_data.items()])
    ...
    for title in st.session_state.innovation_titles:
        ...
        st.markdown(f"### Detail návrhu: {title.strip()}")
        st.markdown(detailed_suggestion)
        st.markdown("---")
    ...
    st.rerun()
     ...
     ai_box("Tímto končí naše interaktivní sezení. Doufám, že detailní analýza a návrhy byly přínosné pro vaše strategické plánování. Pro zahájení nové analýzy obnovte stránku (F5).", title="🎉 Sezení dokončeno")
